...
public:
	...
	bool Change_return() ;
	bool Arm_vehicle() ;
	bool Change_position_mode();
	bool Control_posxyz(float x, float y, float z) ;
	bool Control_pos_vel_yaw(matrix::Vector3f pos, matrix::Vector3f vel, float yaw, float yaw_speed);
...
private:
	float_t xy_rad = 0.1; //旋翼置信半径,在这个范围内认为到达目标点
	float_t fw_xy_rad = 5; //固定翼翼置信半径,在这个范围内认为到达目标点
	float_t z_rad = 0.1;	//高度误差,高度误差在此范围内认为到达目标点
	float_t fw_z_rad = 5;
	float_t yaw_rad = 0.1;
	...
	uORB::Publication<vehicle_local_position_setpoint_s>	_trajectory_setpoint_pub{ORB_ID(trajectory_setpoint)};	//给PID控制器的设定值
};
